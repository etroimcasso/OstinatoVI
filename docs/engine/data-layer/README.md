...
| [battle-commands.md](battle-commands.md) | The 30 battle-command property records plus the command-keyed satellite tables — status/retarget membership masks, the special-handler command lists, the per-command ATB delay and targeting-init byte, and the relic-swap / command→attack pair tables. |
| [battle-tables.md](battle-tables.md) | The battle engine's formula-support and mapping tables — dance-step and Umaro probability ladders, equipment evade boosts, the final-battle chain, dance⇄background pairing, AI script command sizes, throw/slot outcomes, item-type battle flags, and the spell-order offsets. |
| [level-up.md](level-up.md) | Character level progression — the experience, HP, and version-forked MP curves, the swdtech and blitz learn levels, and the level a character joins the party at. |
| [attack-animations.md](attack-animations.md) | The battle-graphics data layer — attack/graphics animation property records, the shared weapon/monster attack-animation record, the packed item jump/throw byte, the usable-item animation references, and the per-monster sprite-overlap shift. |
| [text-corpus.md](text-corpus.md) | The runtime game-text corpus — the `TextCorpus` loader over the per-class `.dat` files, the fixed/pointer record structure and offset tables, and the field/battle/menu codecs that tokenize a record into glyph runs and typed control tokens. |
| [map-metadata.md](map-metadata.md) | The per-map field data — the 33-byte properties record (graphics/tileset/layout indices, scroll, palette, battle background), the parallax and palette/bg animation satellite tables, the new-game NPC-switch seed, and the trigger family: treasures and long/short entrances located through per-map offset tables. |
| [npc-event-triggers.md](npc-event-triggers.md) | The per-map event data — the tile-triggered event scripts and their per-map offset table, each map's startup event, the opaque `EventScriptRef` offset, and the 2,193 nine-byte NPC records (normal / animated / special-graphics variants) with their placement, behaviour, and value-space enums. |
