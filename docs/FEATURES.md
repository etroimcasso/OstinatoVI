...
| build-system | ✅ complete | `features/build-system.md` | CMake + `add_subdirectory(engine)`; 6 targets (lib / binary / smoke + enum + data + spell test runners); GoogleTest 23/24/1; lean binary; dev populate. Engine pin `7c3707a` |
| ci | ✅ complete | `features/ci.md` | 6 self-hosted jobs (5 platform builds + parser unit tests; the ARM64 trio serialized on its shared host); `ci/**` push trigger; runner-local ROM staged + ripped for rip-product e2e |
