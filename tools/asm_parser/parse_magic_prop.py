...
port's typed surfaces, and emits, per the Phase-1.B PLAN (D1/D2/D7/D9/D10):

  * src/data/generated/magic_prop_en_data.inc — one designated-initializer
    AttackProperties row per record (256 records), self-labeling per the
    data-surface discipline; the kAttackPropertiesEn array #includes it.
...
in the prose RAM map), so their bit->name tables below are port-design template
choices mirroring include/ostinato/attack_flags.h — any drift is caught at
...
# --- port-design bit->name tables (mirror include/ostinato/attack_flags.h) ---
...
def decompose_targeting(byte, target, index):
    """PLAN D9: the deterministic targeting-byte decomposition.
    ...
    if remaining:
        raise MagicPropError(
            ...
            "decomposition — escalate per D9".format(index, byte, remaining))
    packed = 0
    for n in names:
        packed |= target[n]
    if packed != byte:
