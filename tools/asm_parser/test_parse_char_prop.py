...
class HelperTests(unittest.TestCase):
    ...
    def test_fixture_row_empty_record(self):
        ...
        rec.bytes = [0] * 22
        self.assertEqual(pcp._fixture_row(rec).strip(), "{},")

    def test_row_comment_forms(self):
        rec = pcp.Record(0x1D, empty=True)
        rec.name = "TERRA_INTRO"
        self.assertEqual(pcp._row_comment(rec), "// [$1D] TERRA_INTRO  (empty)")
        rec2 = pcp.Record(0, empty=False)
        rec2.name = "TERRA"
        self.assertEqual(pcp._row_comment(rec2), "// [$00] TERRA")
...
class RendererTests(unittest.TestCase):
    ...
    def test_fixture_shape(self):
        ...
        self.assertIn("// [$02] GAMMA  (empty)", text)
        self.assertIn(
            "inline constexpr std::array<ExpectedCharacterRecord, 3>", text)
