...
src/field/char_prop.asm and emits:

  * src/data/generated/char_prop_data.inc  — one designated-initializer
    CharacterBaseStats row per record (64 records), every field labeled
    inline; the CharacterBaseStats array #includes it.
...
Structural guarantees, hard-errored at emit time:
  ...
  * the packed trait byte's three fields are disjoint and losslessly recoverable
    (run_factor bits 0-1, level_mod bits 2-3, fixed_equip bit 4);
  * the record count equals the CHAR_PROP index space (64);
...
# Field order of the emitted 22-byte record (== char_prop.asm
...
def _finish(index, regs, symbols, path, lineno):
    ...
    # The three trait fields must be disjoint and losslessly recoverable.
    packed = rf | lm | fe
    if (packed & CharacterTraitsMasks.RUN) != rf \
        ...
        raise ParseError(
            ...
            "fixed={:#04x}) — trait fields must be disjoint".format(rf, lm, fe))
